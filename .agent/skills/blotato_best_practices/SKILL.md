...
**⚠️ CAPTION FORMATTING GOTCHA**: When passing the `text` parameter to `mcp_blotato_blotato_create_post`, **never** use literal escaped newline characters like `\n` in the string (e.g., `text: "Line 1\n\nLine 2"`). This will result in actual literal `\n` text rendering on the social media post. You must use **actual multiline string payloads** (real line breaks) in your tool calls instead.
